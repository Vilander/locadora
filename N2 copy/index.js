...
app.get("/site-locadora/home", function (req, res) {
  conexao.query("SELECT * FROM rac_categoria", function (erro, lista_categorias) {
    if (erro) {
      res.json(erro);
    } else {
...
app.get("/site-locadora/showroom", function (req, res) {
  const idCat = req.query.categoria;

  let sql = "SELECT v.*, c.nomeCat as categoriaVeic FROM rac_veiculos v INNER JOIN rac_categoria c ON v.idCat = c.idCat";
  let parametros = [];

  if (idCat && idCat !== "todas") {
    sql += " WHERE v.idCat = ?";
    parametros.push(idCat);
  }
